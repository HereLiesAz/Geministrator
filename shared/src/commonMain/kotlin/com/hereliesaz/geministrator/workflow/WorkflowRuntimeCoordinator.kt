...
class WorkflowRuntimeCoordinator(
    ...
    suspend fun cycle(project: Project, definition: WorkflowDefinition, state: WorkflowRuntimeState, nowEpochMillis: Long, artifactIdFactory: (TaskRun, ProviderArtifact, Int) -> ArtifactId): WorkflowRuntimeState {
        ...
        for (taskId in newlyFailedTaskIds) nextRun = engine.handleFailure(definition, nextRun, taskId, RetryReason.ProviderFailure, "Executor failed", nowEpochMillis)
    ...
    private suspend fun dispatchSystemExecutors(project: Project, definition: WorkflowDefinition, run: WorkflowRun, nowEpochMillis: Long): WorkflowRun {
        var nextRun = run; val tasks = definition.tasks.associateBy { it.id }
        for ((taskId, taskRun) in run.taskRuns) { val task = tasks[taskId] ?: continue; val executor = taskRun.executor ?: task.effectiveExecutor(); if (!executor.isSystemExecutor() || (taskRun.status != TaskRunStatus.Ready && taskRun.status != TaskRunStatus.Retrying)) continue; val integration = executorIntegrations.integrationFor(executor) ?: continue; nextRun = applyExecution(nextRun, taskId, taskRun, executor, integration.dispatch(TaskExecutorContext(project, definition, nextRun, task, taskRun, executor, nowEpochMillis)), nowEpochMillis) }
        return nextRun
    }

    private suspend fun reconcileSystemExecutors(project: Project, definition: WorkflowDefinition, run: WorkflowRun, nowEpochMillis: Long): WorkflowRun {
        var nextRun = run; val tasks = definition.tasks.associateBy { it.id }
        for ((taskId, taskRun) in run.taskRuns) { if (taskRun.status != TaskRunStatus.Running && taskRun.status != TaskRunStatus.Verifying) continue; val task = tasks[taskId] ?: continue; val executor = taskRun.executor ?: task.effectiveExecutor(); if (!executor.isSystemExecutor()) continue; val integration = executorIntegrations.integrationFor(executor) ?: continue; nextRun = applyExecution(nextRun, taskId, taskRun, executor, integration.reconcile(TaskExecutorContext(project, definition, nextRun, task, taskRun, executor, nowEpochMillis)), nowEpochMillis) }
        return nextRun
    }

    private fun refreshAfterSystemExecution(definition: WorkflowDefinition, run: WorkflowRun, nowEpochMillis: Long): WorkflowRun { val refreshed = WorkflowRunFactory.refreshReadiness(definition, run, nowEpochMillis); return if (refreshed.taskRuns.isNotEmpty() && refreshed.taskRuns.values.all { it.status == TaskRunStatus.Completed }) refreshed.copy(status = WorkflowRunStatus.Completed, updatedAtEpochMillis = nowEpochMillis) else refreshed }

    private fun applyExecution(run: WorkflowRun, taskId: TaskDefinitionId, previous: TaskRun, executor: TaskExecutor, execution: TaskExecutorExecution, nowEpochMillis: Long): WorkflowRun { if (execution.status != previous.status) TaskRunTransitions.requireAllowed(previous.status, execution.status); return run.copy(taskRuns = run.taskRuns + (taskId to previous.copy(status = execution.status, executor = executor, assignedProviderId = null, providerRunId = null, externalRunId = execution.externalRunId ?: previous.externalRunId, artifacts = if (execution.artifacts.isEmpty()) previous.artifacts else execution.artifacts, blockingReason = null, progress = execution.progress ?: if (execution.status == TaskRunStatus.Completed) 1f else previous.progress, progressMessage = execution.progressMessage ?: previous.progressMessage)), updatedAtEpochMillis = nowEpochMillis) }

    private fun preserveArtifactTimestamps(previous: WorkflowRun, reconciled: WorkflowRun): WorkflowRun { val previousArtifacts = previous.taskRuns.values.flatMap(TaskRun::artifacts).associateBy { it.id }; if (previousArtifacts.isEmpty()) return reconciled; return reconciled.copy(taskRuns = reconciled.taskRuns.mapValues { (_, taskRun) -> taskRun.copy(artifacts = taskRun.artifacts.map { artifact -> previousArtifacts[artifact.id]?.let { artifact.copy(createdAtEpochMillis = it.createdAtEpochMillis) } ?: artifact }) }) }

    private fun recoverAvailableSystemExecutors(definition: WorkflowDefinition, run: WorkflowRun, nowEpochMillis: Long): WorkflowRun {
        val definitions = definition.tasks.associateBy { it.id }; var changed = false
        val taskRuns = run.taskRuns.mapValues { (taskId, taskRun) ->
            if (taskRun.status != TaskRunStatus.Blocked || taskRun.blockingReason?.code != WorkflowRunFactory.EXECUTOR_INTEGRATION_UNAVAILABLE) return@mapValues taskRun
            val executor = taskRun.executor ?: definitions[taskId]?.executor ?: return@mapValues taskRun
            if (!executor.isSystemExecutor() || !executorIntegrations.isAvailable(executor)) return@mapValues taskRun
            TaskRunTransitions.requireAllowed(TaskRunStatus.Blocked, TaskRunStatus.Ready); changed = true; taskRun.copy(status = TaskRunStatus.Ready, blockingReason = null)
        }
        return if (changed) run.copy(taskRuns = taskRuns, updatedAtEpochMillis = nowEpochMillis) else run
    }

    private fun blockUnavailableSystemExecutors(definition: WorkflowDefinition, run: WorkflowRun): WorkflowRun { val definitions = definition.tasks.associateBy { it.id }; var changed = false; val taskRuns = run.taskRuns.mapValues { (taskId, taskRun) -> val executor = taskRun.executor ?: definitions[taskId]?.executor; val dispatchable = taskRun.status == TaskRunStatus.Ready || taskRun.status == TaskRunStatus.Retrying || taskRun.status == TaskRunStatus.Running; if (dispatchable && executor != null && executor.isSystemExecutor() && !executorIntegrations.isAvailable(executor)) { changed = true; taskRun.copy(status = TaskRunStatus.Blocked, blockingReason = BlockingReason(WorkflowRunFactory.EXECUTOR_INTEGRATION_UNAVAILABLE, "${executor.displayLabel()} is not available in this runtime."), progress = null, progressMessage = null) } else taskRun }; return if (changed) run.copy(taskRuns = taskRuns) else run }
