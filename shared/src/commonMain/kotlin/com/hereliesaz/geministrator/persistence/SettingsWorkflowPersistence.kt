...
class SettingsWorkflowPersistence(
    ...
    override val projects: ProjectRepository = object : ProjectRepository {
        override suspend fun put(project: Project) = update { snapshot -> snapshot.copy(projects = snapshot.projects.upsert(project) { it.id == project.id }) }
    ...
    override val definitions: WorkflowDefinitionRepository = object : WorkflowDefinitionRepository {
        override suspend fun put(definition: WorkflowDefinition) = update { snapshot -> snapshot.copy(definitions = snapshot.definitions.upsert(definition) { it.id == definition.id }) }
        override suspend fun get(id: WorkflowDefinitionId): WorkflowDefinition? = read().definitions.firstOrNull { it.id == id }
    ...
    override val runs: WorkflowRunRepository = object : WorkflowRunRepository {
        override suspend fun put(run: WorkflowRun) = update { snapshot -> snapshot.copy(runs = snapshot.runs.upsert(run) { it.id == run.id }) }
        override suspend fun get(id: WorkflowRunId): WorkflowRun? = read().runs.firstOrNull { it.id == id }
        override suspend fun byProject(projectId: ProjectId): List<WorkflowRun> = read().runs.filter { it.projectId == projectId }.sortedByDescending { it.updatedAtEpochMillis }
    }

    override val events: WorkflowEventRepository = object : WorkflowEventRepository {
        override suspend fun append(event: WorkflowEvent) = update { snapshot -> snapshot.copy(events = snapshot.events + event) }
        override suspend fun forRun(workflowRunId: WorkflowRunId): List<WorkflowEvent> = read().events.filter { it.workflowRunId == workflowRunId }.sortedBy { it.occurredAtEpochMillis }
    }

    override val roles: RoleRepository = object : RoleRepository {
        override suspend fun put(role: RoleDefinition) = update { snapshot -> snapshot.copy(roles = snapshot.roles.upsert(role) { it.id == role.id }) }
    ...
    override val artifacts: ArtifactRepository = object : ArtifactRepository {
        override suspend fun put(artifact: ArtifactRef) = update { snapshot -> snapshot.copy(artifacts = snapshot.artifacts.upsert(artifact) { it.id == artifact.id }) }
    ...
    override val approvalGates: ApprovalGateRepository = object : ApprovalGateRepository {
        override suspend fun put(gate: ApprovalGate) = update { snapshot -> snapshot.copy(approvalGates = snapshot.approvalGates.upsert(gate) { it.id == gate.id }) }
        override suspend fun get(id: ApprovalGateId): ApprovalGate? = read().approvalGates.firstOrNull { it.id == id }
    ...
    suspend fun clearWorkflowData() { mutex.withLock { settings.remove(storageKey) } }
    ...
    private fun readUnlocked(): PersistenceSnapshot {
        val encoded = settings.getStringOrNull(storageKey) ?: settings.getStringOrNull(LEGACY_STORAGE_KEY_V1) ?: return PersistenceSnapshot()
        ...
        if (migrated.version < 2) {
            migrated = migrated.copy(
                ...
                definitions = migrated.definitions.map { definition ->
                    definition.copy(tasks = definition.tasks.map { task ->
                        if (task.executor != null || task.roleId == null) task else task.copy(executor = TaskExecutor.RoleAgent(task.roleId))
                    })
                },
                runs = migrated.runs.map { run ->
                    run.copy(taskRuns = run.taskRuns.mapValues { (_, taskRun) ->
                        if (taskRun.executor != null || taskRun.assignedRoleId == null) taskRun else taskRun.copy(executor = TaskExecutor.RoleAgent(taskRun.assignedRoleId))
                    })
                },
            )
        }
    ...
    companion object {
        ...
        val defaultJson: Json = Json { encodeDefaults = true; ignoreUnknownKeys = true; classDiscriminator = "type"; allowStructuredMapKeys = true }
