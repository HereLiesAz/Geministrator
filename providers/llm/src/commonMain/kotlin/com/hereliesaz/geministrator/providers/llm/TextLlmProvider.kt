...
open class TextLlmProvider(
    ...
    override suspend fun start(request: AgentTaskRequest): AgentRunHandle {
        val result = api.generate(renderPrompt(request))
    ...
    override fun observe(runId: ProviderRunId): Flow<AgentEvent> = flow {
        ...
        if (session.request.requirePlanApproval) {
            emit(
                AgentEvent.PlanGenerated(
                    runId = runId,
                    summary = session.result.text.take(MAX_PLAN_PREVIEW_CHARS),
                ),
            )
        ...
        } else if (session.phase.value == TextLlmSessionPhase.Cancelled) {
            emit(AgentEvent.Failed(runId, "$displayName session cancelled"))
            return@flow
        }
        ...
        emit(
            AgentEvent.ArtifactProduced(
                ...
                artifact = ProviderArtifact(
                    ...
                    textContent = session.result.text,
        ...
        if (session.result.inputTokens != null || session.result.outputTokens != null) {
            emit(
                AgentEvent.UsageReported(
                    runId = runId,
                    inputTokens = session.result.inputTokens,
                    outputTokens = session.result.outputTokens,
                ),
            )
        }
