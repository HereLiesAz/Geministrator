...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('geministrator.start', () => {
        if (orchestratorPanel) {
            orchestratorPanel.reveal(vscode.ViewColumn.Two);
            return;
        }
        orchestratorPanel = vscode.window.createWebviewPanel('geministrator', 'Geministrator', vscode.ViewColumn.Two, {
            enableScripts: true,
        ...
        orchestratorPanel.webview.onDidReceiveMessage(message => {
...
function runCliProcess(prompt) {
    ...
    orchestratorProcess = (0, child_process_1.spawn)(command, args);
