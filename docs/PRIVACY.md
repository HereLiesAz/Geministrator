# Privacy Policy for The Haive

**Effective date: September 7, 2026**

This policy describes the current open-source builds of **The Haive**, application ID `com.hereliesaz.haive`.

The Haive is a workflow orchestration application. It can store workflow information locally and, when you choose to use an external executor or provider, send the information needed to perform that work to the service you selected.
...
- The Haive does **not** sell personal information.
...
Depending on how you use the app, The Haive may store:
...
The Haive is designed to work with external services. Jules is the first provider integration, and future versions may support additional AI providers, GitHub Actions, deployment systems, test runners, and other executors.

When you assign work to an external service, The Haive may send information required for that task, such as:
...
Repository-backed work may require an external provider to access a repository or a source already connected to that provider. The Haive should request and transmit only the repository/context needed for the selected task.

The Haive itself is not intended to upload an entire repository to a separate analytics or data-collection service.
...
If The Haive later adds optional diagnostics or analytics, this policy must be updated before that behavior is treated as part of the product.

## Data retention and deletion

Local workflow data remains until The Haive removes it or you clear the application's data/storage. On the web, clearing site data for The Haive removes the browser-local persistence used by the current implementation.
...
The Haive shares task data only as needed to use services/executors you choose. The current application does not sell personal data or share workflow content with advertisers.

## Children's privacy

The Haive is a software-development tool and is not directed to children under 13. It is not designed to knowingly collect personal information from children.
...
Material changes to The Haive's data handling should be accompanied by an update to this document and its effective date.

## Contact

The Haive is an open-source project maintained by **HereLiesAz**. Privacy questions and reports can be raised through the repository at `HereLiesAz/haive` on GitHub.
